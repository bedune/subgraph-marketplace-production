    ...
    if (outcome.logs != null && outcome.logs.length > 0) {
      ...
      if (outcome.logs[0].split(":")[0] == '{"type"') {
        ...
        if (parsed.kind == JSONValueKind.OBJECT) {
          ...
          // Standard receipt properties
          mints.blockTime = BigInt.fromU64(
            blockHeader.timestampNanosec / 1000000
          );
          mints.blockHeight = BigInt.fromU64(blockHeader.height);
          mints.blockHash = blockHeader.hash.toBase58();
          mints.predecessorId = receipt.predecessorId;
                ...
                for (let m = 0; m < paramObject.entries.length; m++) {
                  ...
                  switch (true) {
                    ...
                    case key == "receiver_id":
                      mints.receive_id = paramObject.entries[
                        m
                      ].value.toString();
                      break;
                  }
  ...
  if (functionCall.methodName == "nft_create_series") {
    ...
    if (outcome.logs != null && outcome.logs.length > 0) {
      for (let x = 0; x < outcome.logs.length; x++) {
        ...
        // Standard receipt properties
        series.blockTime = BigInt.fromU64(
          blockHeader.timestampNanosec / 1000000
        );
                  ...
                  for (let m = 0; m < paramObject.entries.length; m++) {
                    ...
                    switch (true) {
                      case paramKey == "token_series_id":
                        series.token_series_id = paramObject.entries[
                          m
                        ].value.toString();
                        break;
                      case paramKey == "token_metadata":
                        if (
                          paramObject.entries[m].value.kind ==
                          JSONValueKind.OBJECT
                        ) {
                          let metaObject = paramObject.entries[
                            m
                          ].value.toObject();
                          for (let j = 0; j < metaObject.entries.length; j++) {
                            ...
                            switch (true) {
                              case metaKey == "title":
                                series.title =
                                  metaObject.entries[j].value.kind !=
                                  JSONValueKind.NULL
                                    ? metaObject.entries[j].value.toString()
                                    : null;
                                break;
                              ...
                              case metaKey == "copies":
                                series.copies =
                                  ...
                                  metaObject.entries[j].value.kind ==
                                    ...
                                    ? metaObject.entries[j].value.toBigInt()
                                    : metaObject.entries[j].value.kind !=
                                        JSONValueKind.NULL &&
                                      metaObject.entries[j].value.kind ==
                      ...
                      case paramKey == "creator_id":
                        series.creator_id =
                          paramObject.entries[m].value.kind !=
                          JSONValueKind.NULL
                            ? paramObject.entries[m].value.toString()
                            : null;
                        break;
                      case paramKey == "price":
                        series.price =
                      ...
                      case paramKey == "royalty":
                        if (
                          ...
                          let royaltyObject = paramObject.entries[
                            m
                          ].value.toObject();
                          for (
                            let p = 0;
                            p < royaltyObject.entries.length;
                            p++
                          ) {
                            let royalties = new Royalty(`${receiptId}-${p}`);
                            royalties.account = royaltyObject.entries[
                              p
                            ].key.toString();
                            royalties.amount = royaltyObject.entries[
                              p
                            ].value.toBigInt();
                            royalties.save();
              ...
              if (entry.entries[i].value.kind == JSONValueKind.OBJECT) {
                ...
                for (let m = 0; m < paramObject.entries.length; m++) {
                  ...
                  switch (true) {
                    ...
                    case paramKey == "price":
                      series.price =
                        paramObject.entries[m].value.kind != JSONValueKind.NULL
                          ? BigInt.fromString(
                              paramObject.entries[m].value.toString()
                            )
                          : null;
                ...
                for (let m = 0; m < paramObject.entries.length; m++) {
                  ...
                  switch (true) {
                    ...
                    case paramKey == "approval_id":
                      series.approval_id = paramObject.entries[
                        m
                      ].value.toString();
                      break;
                    case paramKey == "nft_contract_id":
                      series.nft_contract_id = paramObject.entries[
                        m
                      ].value.toString();
                      break;
                    case key == "token_id":
                    ...
                    case paramKey == "started_at":
                      series.started_at =
                        paramObject.entries[m].value.kind != JSONValueKind.NULL
                          ? paramObject.entries[m].value.toBigInt()
                          : null;
                      break;
                  ...
                  for (let k = 0; k < dataObject.entries.length; k++) {
                    ...
                    switch (true) {
                      case key == "authorized_id":
                        mints.authorized_id = dataObject.entries[
                          k
                        ].value.toString();
                        break;
                      case key == "old_owner_id":
                        mints.old_owner_id = dataObject.entries[
                          k
                        ].value.toString();
                        break;
                      case key == "new_owner_id":
                        mints.new_owner_id = dataObject.entries[
                          k
                        ].value.toString();
                        break;
                      case key == "token_ids":
                ...
                for (let m = 0; m < paramObject.entries.length; m++) {
                  ...
                  switch (true) {
                    ...
                    case paramKey == "is_non_mintable":
                      series.is_non_mintable = paramObject.entries[
                        m
                      ].value.toBool();
                      break;
                  }
                  ...
                  for (let m = 0; m < paramObject.entries.length; m++) {
                    ...
                    switch (true) {
                      case paramKey == "token_id":
                        series.token_series_id = paramObject.entries[m].value
                          .toString()
                          .split(":")[0];
                        series.token_id = paramObject.entries[m].value
                          .toString()
                          .split(":")[1];
                        break;
                      case paramKey == "sender_id":
                        series.sender_id = paramObject.entries[
                          m
                        ].value.toString();
                        break;
                      case paramKey == "receiver_id":
                        series.receive_id = paramObject.entries[
                          m
                        ].value.toString();
                        break;
                    }
